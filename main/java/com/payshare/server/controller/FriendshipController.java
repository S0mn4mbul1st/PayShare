...
import com.payshare.server.dto.friendship.FriendshipCollectionReadDto;
import com.payshare.server.dto.friendship.FriendshipCreateDto;
import com.payshare.server.dto.friendship.FriendshipReadDto;
...
import com.payshare.server.service.FriendshipService;
import com.payshare.server.service.UserAccountService;
...
public class FriendshipController {
    ...
    public ResponseEntity<?> getAllFriendships(Authentication authentication) {
        ...
        List<FriendshipReadDto> confirmedFriendships = friendshipService.getAllConfirmedFriendshipsForPerson(person)
                .stream()
                .map(this::assembleDto)
                .collect(Collectors.toList());
        List<FriendshipReadDto> pendingFriendships = friendshipService.getAllPendingFriendshipForPerson(person).stream()
                .map(this::assembleDto)
                .collect(Collectors.toList());
        List<FriendshipReadDto> receivedRequests = friendshipService.getAllFriendshipRequestsForPerson(person).stream()
                .map(this::assembleDto)
                .collect(Collectors.toList());
    ...
    public ResponseEntity<?> sendFriendshipRequest(@RequestBody FriendshipCreateDto friendshipCreateDto,
        ...
        try {
            ...
            Friendship requestedFriendship = friendshipService.sendFriendshipRequest(requestingPerson, targetPerson);
